...
func lunarCsvtojson(file: String) {
	...
	do {
	    ...
	    for item in values {
	        let reg = try Regex("\",")
	        let row = item.split(separator: reg).map {String($0).replacingOccurrences(of: "\"", with: "")}
	        ...
	        ISBN: Int(row[13]) ?? 0,
	        EAN: Int(row[14]) ?? 0,
	        Writer: (String(row[15])).split(separator: ",").map {String($0.trimmingPrefix(" "))},
	        Artist: (String(row[16])).split(separator: ",").map {String($0.trimmingPrefix(" "))},
	        CoverArtist: row[17],
	        ...
	        ImageUrl: URL(string: row[53]) ?? URL(string: "https://www.google.com")!,
	        Returnable: row[54],
	        ReturnWindow: row[55]
	        )
	...
	do {
		...
		jsonfile.append(component: "comic_data.json")
	} catch {
